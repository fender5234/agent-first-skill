...
**Блок — это не файл.** Частое заблуждение: «манифест — это список файлов, новые файлы туда дописываются». Нет. Блок крупнее файла: это папка или модуль с одной ответственностью.

- Добавил файл внутрь существующего блока → в манифесте **не меняется ничего**. Максимум — строка в `notes`, и только если файл ключевой.
- Появилась новая папка-блок → добавляется запись, и орфан-детектор сам об этом напомнит.

Это не деталь оформления, на этом держится вся конструкция. В проекте на 168 исходных файлов получается ~70 блоков. Если вести манифест по файлам — 168 записей, правка при каждом коммите, разъедется за неделю. Соотношение «файлов на блок» в норме 2-10; если оно ушло к 1, нарезка слишком мелкая.
...
- **push в дефолтную ветку деплоит** → ветка + локальный merge + push. PR-гейт НЕ
  ставить. Гард `check-not-main` **ставить**.
- **PR-based** → ветка + PR + merge только после зелёного CI и ревью в чистой
  сессии. Гард `check-not-main` ставить.

**`check-not-main` совместим с деплоем из дефолтной ветки** — очевидная догадка
здесь неверна. Это **pre-commit** хук: он отказывает `git commit`, когда HEAD на
дефолтной ветке. На `git push` он не запускается вовсе, то есть деплой заблокировать
физически не может. При работе «ветка → `merge --ff-only` → push» коммита на
дефолтной ветке не создаётся, и хук не срабатывает ни разу. Слияния тоже мимо: git
вызывает для них `pre-merge-commit`, а не `pre-commit`, и фреймворк по умолчанию
ставит только второй.

Ответ про модель деплоя решает судьбу **PR-гейта**, а не гарда ветки.
```
...
## 3.1. Версия методологии в проекте и апгрейд

Скилл живёт на машине — `~/.claude/skills/agent-first-setup/`. В репозиторий проекта
он **не попадает никогда**. Туда попадают порождённые им артефакты:
`documentation/*.yaml`, `documentation/validate.py`, `scripts/validate.sh`,
`.pre-commit-config.yaml`, секции в `CLAUDE.md`.

Это **копии**, и они перестают следить за скиллом в момент записи. Дальше проект
наращивает в них своё: в одном появляется проверка публичного API виджетов, в
другом — сверка токенов с `globals.css`. Три потомка одного шаблона расходятся, и
это правильно — форк здорового проекта ценнее шаблона.

Обратная сторона: улучшение шаблона само в форки не переезжает.

**Отсюда поле версии:**

```yaml
meta:
  af_version: 1.4.0   # отсутствует = 1.0.0
```

Без него апгрейд не от чего отсчитывать, и «обнови проект под новый скилл»
вырождается в повторный setup поверх живого проекта — то есть в перезапись
проектных проверок.

**Правило апгрейда одно и оно жёсткое: сливать, а не перезаписывать.** Новая
проверка добавляется следующим свободным номером; существующие не трогаются и не
перенумеровываются — их id встречаются в сообщениях коммитов, чек-листах ревью и
прозе `CLAUDE.md`.

**Секции `CLAUDE.md` чинятся по пунктам, а не по заголовкам.** Типичный отказ —
секция на месте, но потеряла половину содержимого: чек-лист Self-Check без
Test-First по заголовку находится, значит при append-if-missing будет пропущен, и
дыра переживёт апгрейд.

Полная процедура — раздел `Upgrade Workflow` в `SKILL.md`.
...
| **3** | CI check — `validate.py` на PR | Появился второй разработчик ИЛИ обходишь pre-commit |
...
**Уровень 3 — это одна команда, а не пайплайн.** Шаблон `ci-validate.yml` гоняет
только `python documentation/validate.py` на PR: секунды, без установки
зависимостей и сборки. Полный пайплайн — установка, линт, typecheck, тесты,
миграции, сборка — это другое решение с другой ценой, и скилл его не поставляет.
Не читать опциональный шаблон как рекомендацию строить второе.

**Полный CI и ревью в чистой сессии ловят разное:**

| Класс дефекта | Полный CI | Ревью в чистой сессии |
|---|---|---|
| Незакоммиченный файл, зависимость мимо манифеста, поведение от env | ловит | **по умолчанию пропускает** — то же рабочее дерево; закрывается пунктом про чистый checkout в шаге 5 ревью |
| Поломка на другой ОС или версии рантайма | ловит | пропускает |
| Ложное заявление, что тест что-то защищает | пропускает — все гейты зелёные | ловит (шаг 7) |
| Молча нереализованный acceptance-пункт | пропускает | ловит |
| Нарушенное процессное правило | пропускает | ловит (шаг 8) |

Из реальной практики: команда гоняла оба, нашла полный пайплайн медленным
относительно того, что он добавлял поверх ревью, и отключила — оставив ревью
единственным гейтом. Размен защитим **при условии**, что пункт про чистый checkout
действительно выполняется: это единственная колонка, которую ревью иначе оставляет
пустой.

Не путать пайплайн, **гейтящий мёржи**, с пайплайном, который **деплоит**. Отказ от
первого ничего не говорит про второй.
...
Перед и во время реализации — использовать sequential-thinking для любого нетривиального решения. Касается: ADR-решений, выбора подхода к реализации, оценки trade-offs, отладки сложных проблем, стратегии рефакторинга, и выборов по ходу написания кода.

**Структура:** проблема → варианты → trade-offs → решение. Не "выстреливать первой мыслью".

**Если в процессе реализации возникает развилка ("сделать X или Y?")** — остановиться и использовать sequential-thinking прежде чем продолжить.
...
**Всегда покрывать edge cases явно:**
- Null / undefined / пустые входные данные
- Пустые массивы и коллекции
- Ошибки сети, таймауты, API возвращает 4xx/5xx
- Невалидный или некорректный пользовательский ввод
- Конкурентные запросы / race conditions (если применимо)
- Граничные значения (0, отрицательные, max int, пустая строка)
...
### 9.6. Соблюдать единые Naming Conventions для удобства агента

Перед созданием любого файла, переменной или функции — grep проект на существующие паттерны именования и следовать им точно.

**Файлы:**
- Папка фичи: существительное во множественном числе (orders/, users/, payments/)
- Компоненты: PascalCase, совпадает с доменом фичи (OrderList.tsx, OrderCard.tsx)
- Хуки: use + название фичи (useOrders.ts, useCart.ts)
- API: домен + Api (orderApi.ts, userApi.ts)
- Типы: домен + Types (orderTypes.ts, userTypes.ts)
- Тесты: то же имя + .test (OrderList.test.tsx)

**Greppability (поиск по проекту):**
- Один домен = одно корневое слово везде (order, а не вперемешку purchase/item/product)
- Без сокращений в именах файлов (usr → user; auth допустимо — это доменный термин)
- Без generic-имён (utils.ts, helpers.ts, misc.ts) — всегда с доменным префиксом (orderUtils.ts)

**Предсказуемость:**
- Если паттерн уже существует — следуй ему точно, не изобретай синонимы
- Перед именованием — grep проект на существующие конвенции
- Именование новой фичи должно совпадать с паттерном существующих фич

**Почему это критично для agent-first:**
- Агент находит связанный код через grep по одному корневому слову
- Предсказуемые имена = агент не открывает файл чтобы понять что внутри
- Синонимы ломают grep: если orders в одном месте и purchases в другом — агент не найдёт всё связанное
- Generic-имена (utils.ts) скрывают назначение — агент не знает что там без чтения

### 9.7. Применять базовые правила Security при написании кода

Безопасный код пишется сразу, а не добавляется потом. Агенты генерируют код на скорости — уязвимости тоже.

**Всегда:**
- Параметризованные запросы для SQL/NoSQL — никогда не конкатенировать пользовательский ввод в запрос
- Валидировать и санитизировать весь пользовательский ввод на точках входа
- Секреты хранить в переменных окружения или secret manager — никогда не хардкодить в коде
- HTTPS для всех внешних коммуникаций

**Никогда:**
- Логировать чувствительные данные (пароли, токены, PII, полные номера карт)
- CORS wildcard (*) в продакшне
- Коммитить секреты, .env файлы или credentials в репозиторий
- Полагаться только на клиентскую валидацию — всегда валидировать серверную сторону

**При добавлении зависимостей:**
- Проверить на известные уязвимости перед добавлением (npm audit, pip-audit и т.д.)
- Предпочитать хорошо поддерживаемые пакеты с недавними обновлениями

**Почему это критично для agent-first:**
- Агент генерирует код быстро — уязвимости масштабируются с той же скоростью
- Без явных правил агент может выбрать удобный, но небезопасный паттерн (конкатенация SQL вместо параметризации)
- Захардкоженные секреты в коде — одна из самых частых ошибок агентов
- Security-проверка в Self-Check ловит то, что проскочило в процессе

### Как семь правил работают вместе
...
context7 даёт актуальные знания
  → sequential-thinking структурирует решение
    → Feature-Based Architecture задаёт структуру проекта
      → Naming Conventions обеспечивают единообразие и поиск
        → Security задаёт базовую безопасность кода
...
Код сразу пишется качественно, безопасно, в правильной структуре, с единообразными именами, с гарантией поведения. Self-Check в конце цикла ловит нарушения, которые проскочили в процессе.
...
1. **Шаг 1:** один YAML-манифест на самый активный слой + `validate.py` + pre-commit. Начинай отсюда, потому что у самого активного слоя границы блоков обычно очевиднее всего.
2. **Шаг 2:** расширяй на остальные слои + `project.yaml` — как только для каждого выбран принцип нарезки. Календарной паузы не требуется.
...
meta:
  ...
  # Версия методологии, из которой порождены артефакты проекта. Ставится
  # установкой, поднимается Upgrade Workflow — руками не править.
  # Отсутствует = 1.0.0 (артефакты старше этого поля). См. §3.1.
  af_version: 1.4.0
...
### Шаг 5: Принцип нарезки следующего слоя
Единственное, что должно предшествовать расширению. Календарная пауза — не должна.

Вопрос настоящий не везде. Там, где папки уже имеют чистые границы (`src/widgets/<name>/` с `index.ts`), ответ «блок = папка» очевиден и обсуждать нечего. Проблемные случаи, где нужно решение:

- **Папка склеивает несвязанное.** `lib/` из восьми модулей — SQLite, SMTP, расчёт цены, обрезка строк. Один блок «lib» получит summary уровня «разные утилиты», то есть бесполезный. Здесь блок = модуль, и несколько блоков делят один `code_path`, различаясь `entry`. Это законно и нормально.
- **Много тонких обёрток вокруг одного рендерера.** 12 страниц, каждая в 5 строк, отрисованных одной функцией из данных. Это один блок, а не 12 клонов; контент живёт в data-файле, а не в блоке.
- **Блоки на разной глубине.** Если `code_path` одного блока указывает на родителя других (`src/app` при наличии `src/widgets/*`), орфан-детектор начнёт помечать сиротами покрытые директории. Лечится тремя строками в `validate.py`: не считать сиротой директорию, которая сама является родителем покрытых путей.

### Шаг 6: Обкатка на реальных задачах
Расширять можно сразу, но *полезность* манифеста проверяется только использованием. На ближайших задачах смотри:
- Какие поля хочется дозаполнить (`gotchas`, `notes`)
- Лезет ли агент грепать репо вместо чтения YAML — это признак, что полей не хватает
- Где валидатор реально поймал ошибку, а где только шумел

## Чего избегать при миграции

- **Не документировать слой, для которого не выбран принцип нарезки.** Раньше здесь стояло «не документировать всё разом», и аргументом была стоимость: 50+ блоков за раз — это дни ручной работы, после которых манифест никто не поддерживает. С параллельной инвентаризацией субагентами аргумент отпал — слой в 37 блоков размечается за минуты, и это заполненные summary, а не заглушки. Остался другой довод, настоящий: неверная нарезка, размноженная на три слоя, дороже любой экономии. Решай нарезку, а не считай дни.
- **Не генерировать блоки заглушками.** Манифест из `summary: TODO` хуже отсутствующего: он выглядит покрытием, не будучи им. Нет времени написать осмысленные summary на слой — не начинай этот слой.
...
3. Следуешь Шагам 1-6 (инвентаризация → манифест → валидатор → следующий слой)
4. Для каждого следующего слоя решаешь принцип нарезки — и расширяешь сразу, без календарной паузы
**Твоя роль:** выбираешь стартовый слой и принцип нарезки для каждого следующего.
...
[ ] 8. Self-Check — проверить код на соответствие cross-cutting правилам:
       ...
       - [ ] sequential-thinking: использован для всех нетривиальных решений во время реализации?
       ...
       - [ ] Naming: новые имена следуют существующим паттернам проекта (grep перед именованием)?
       - [ ] Naming: один домен = одно корневое слово, нет смешения синонимов?
       - [ ] Naming: нет generic-имён (utils.ts, helpers.ts) без доменного префикса?
       - [ ] Security: нет захардкоженных секретов (API ключи, пароли, токены)?
       - [ ] Security: нет конкатенации строк в SQL/NoSQL запросах — используются параметризованные запросы?
       - [ ] Security: пользовательский ввод валидируется/санитизируется?
       - [ ] Security: нет чувствительных данных в логах или сообщениях об ошибках?
       - [ ] Security: нет CORS wildcard (*) в продакшн-конфиге?
       - [ ] Security: новые зависимости проверены на известные уязвимости?
       - [ ] Test-First: тесты есть и проходят для новых/изменённых модулей?
       - [ ] Test-First: edge cases покрыты (null, пустые, ошибки, невалидный ввод, границы)?
